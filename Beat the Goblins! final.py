...
"""
2D platform game. Arrow keys to move and jump, space to shoot. Starts of with 1 goblin who moves slowly. As levels 
increase, goblins speed up (up to a maximum speed). Number of goblins increase every 5 levels. Highest level is set to
30 by default, but increasing this is as simple as increasing the 'max_level' variable, and everything else is rescaled.
Player has a chance to buy various power-ups in the pause screen, such as a shield, goblin freeze, more bullets & health

"""

"""
Small glitches (fix later if bothered):
Shield: shield is removed as soon as enemy collides with player. If enemy is moving too slowly, then it is still in
contact with player after shield is removed, hence they collide. However, this only occurs in the low levels, where it 
is very easy to dodge the goblin.

Scaling: Rarely (if the window is rescaled wayyy too much and often), somethings might go out of alignment. This is
extremely rare and only happens when window is rescaled a lot (for debugging), and so is not something user is likely to 
experience. No idea why this happens. But a button 'fix' was introduced to allow player to reposition everything in 
worst case scenario.

Button: Buttons sometimes require clicking several times before the click registers. No idea why this happens, but they 
eventually work anyways, so not a big problem.
"""
...
class Player(object):
    ...
    def __init__(self, x, y, vel):
        self.x = x
        self.y = y
        self.initial_pos = (self.x, self.y)  # store initial position, use for rescaling, respawning, jumping, etc
        ...
        self.restart = False
        self.has_shield = True  # shield protects player from one collision
        ...
        self.score_increment = 1  # inc score by this when goblin is shot successfully
    ...
    def do_jump(self):
        # models jumping as a quadratic function, jump_value = x, height = y: so goes from max to -max while doing y^-2
        if self.jump_value >= -self.max_jump_value:
            ...
            if self.jump_value < 0:
                if self.y + (self.jump_value ** 2) < self.initial_pos[1]:  # doesn't go too high
                    self.y += (self.jump_value ** 2)
                else:
    ...
    def draw(self, showing_timer=False):
        ...
        if self.right:
            if self.jump:
                ...
                win.blit(self.walkRight[self.walkcount // image_duration], (int(self.x), int(self.y)))
            ...
            if self.jump:
                ...
                win.blit(self.walkLeft[self.walkcount // image_duration], (int(self.x), int(self.y)))
    ...
    def draw_health_bar(self):
        ...
        self.hitbox = (int(self.x) + 20, int(self.y) + 15, 23, 45)
        offset = 10
        width = 40
        height = 10
        pygame.draw.rect(win, colours["black"], (self.hitbox[0] - offset, self.hitbox[1] - offset, width, height))
        pygame.draw.rect(win, colours["red"], (self.hitbox[0] - offset, self.hitbox[1] - offset, int(width -
    ...
    def reset_position(self):
        self.x = 0.5 * win_width  # use to move character (and goblins) to prevent continuous collisions
        self.y = y_factor_all * win_height

    def hit(self):
        # called when player is hit by goblin and when without a shield
        self.walkcount = 0
...
class Goblins(object):
    ...
    def create_goblin(cls):
        ...
        spacing = 0.06  # spawn goblins on opposite ends of screen, with a spacing of 0.06 * win_width
        for i in range(1, (veronian.level - 1) // 5 + 2):
            velocity = 1.5 * veronian.level * ((-1) ** (i + 1))  # makes goblins on either end move in opp directions
            if i % 2 != 0:
...
def fix_position():
    # used by btn to fix the rare glitch of characters getting rescaled wrongly
...
def display_timer(time):
    # displays a countdown for the given time in the centre of the screen
    for i in range(time):
...
def check_input(check_quit=False, check_pause=False, check_retry=False, check_resize=False, check_enter=False,
                check_all=False):
    # check for retry/ pause/ quit/ video resize/ enter key
    global previous_win_width, previous_win_height, win_width, win_height, running, paused, pause_ticker, win, pause_clicked
    ...
    for event in pygame.event.get():
        if event.type == pygame.VIDEORESIZE and check_resize:
            previous_win_width, previous_win_height = win.get_width(), win.get_height() # use to scale
...
def adjust_freeze_time():
    # if goblins are frozen and player moves to pause screen, this is called to ensure the time spent on pause screen
    # is not counted as the time goblins spend frozen
...
def wrap_text(text, font, colour, pos, centralisex=False, centralisey=False, width=70):
    ...
    # centralisex, centralisey makes the 'pos' specified as the centre of the text in the horizontal, vertical direction
    # if they are false, then the text is blitted from the 'pos' specified
    ...
    for line in text:
        ...
        win.blit(text2, (pos[0] if not centralisex else (win_width - text2.get_width()) // 2,
                         int(pos[1] + font.size('a')[1] * line_count)))
        line_count += 1


def create_text(text, font_size, colour, x_factor, y_factor, centralisex=False, centralisey=False):
    # create the various text objects for gamewindow, pausewindow, buttons, etc
    # centralisex, centralisey makes the 'pos' specified as the centre of the text in the horizontal, vertical direction
    # if they are false, then the text is blitted from the 'pos' specified
    ...
    pos = (int(x_factor * win_width)if not centralisex else (win_width - make_text.get_width()) // 2,
           int(y_factor * win_height) if not centralisey else (win_height - make_text.get_height()) // 2)
...
def draw_texts(score=False, level=False, shield=False, bullets=False, money=False, pause=False):
    ...
    if pause:
        create_text(*texts["pause"])
    if score:
        create_text(*texts["score"])
    if level:
        create_text(*texts["level"])
    if shield:
        create_text(*texts["shield"])
    if bullets:
        create_text(*texts["bullets"])
    if money:
        create_text(*texts["money"])
...
def rescale():
    # rescale everything (player and enemy current and initial positions, velocities, background and font)
...
def load_texts(default_font_size, title_font_size, x_factor_end, x_factor_start, y_factor_top, y_factor_spacing):
    # note: texts and buttons need to be reloaded to adjust for any resizing since they were last created
    global texts
...
try:
    ...
    paused = False
    y_factor_all = 0.85  # all objects should be at height 0.85 * win_height to match the background
    pause_ticker = 0
